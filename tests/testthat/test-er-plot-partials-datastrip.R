test_that("build_datastrip_jitter returns geom + coord + yscale", {
  ...
  expect_no_error(p1 |> er_plot_show_datastrip(style = "jitter"))
  expect_no_error(p2 |> er_plot_show_datastrip(style = "jitter"))

  p1 <- p1 |> er_plot_show_datastrip(style = "jitter")
  p2 <- p2 |> er_plot_show_datastrip(style = "jitter")

  config1 <- p1$part$strip$config
  config2 <- p2$part$strip$config
  ...
  expect_no_error(do.call(build_datastrip_jitter, args1))
  expect_no_error(do.call(build_datastrip_jitter, args2))

  p1_out <- do.call(build_datastrip_jitter, args1)
  p2_out <- do.call(build_datastrip_jitter, args2)
