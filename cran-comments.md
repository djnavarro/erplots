...
`erplots` is not yet on CRAN; this is a resubmission responding to
reviewer feedback on the 0.1.1 submission (see "Resubmission summary"
below for the changes made). Summary of the package as per DESCRIPTION:
...
Local tests, CI checks, R-hub checks, and win-builder checks are all
reported below, with no issues. There is also a brief discussion of the
relationship between `erplots` and the companion package `erglm` now on
CRAN (as the first package that implements the interface).
...
* R-hub (`R-hub` workflow, run on 2026-08-27,
  <https://github.com/djnavarro/erplots/actions/runs/33069116463>):
  ...
  * `nosuggests` (Fedora 42, none of `Suggests` installed): 0 test
    failures (1038 passed, 52 skipped) and 0 example failures, but
    ...
    package bug.
* win-builder (submitted 2026-08-27 via `devtools::check_win_devel()`/
  `check_win_release()`/`check_win_oldrelease()`): R-devel (r90448),
  R-release (4.6.1), and R-oldrelease (4.5.3) all `Status: 1 NOTE` --
  only the standard "New submission" NOTE, nothing else. Unlike the
  0.1.1 submission, none of the three logs report the earlier
  "Package suggested but not available for checking: 'erglm'" `INFO`
  line this time round.
* `revdepcheck::revdep_check()`, rerun on 2026-08-27 against this
  submission's source: 0 new problems (see "Downstream companion
  package and `Suggests`" below).

## R CMD check results

0 errors | 0 warnings | 1 note

* The one NOTE is the standard `New submission` NOTE that win-builder
  and CRAN's own incoming checks raise for every first submission.
...
* `erglm` is therefore also this submission's one existing CRAN reverse
  dependency (via `Suggests`). Rechecked with `revdepcheck::revdep_check()`
  against this submission's source on 2026-08-27: 0 new problems (see
  `revdep/cran.md`). Two further companion packages, `emaxnls` and
  `ertte`, were also checked as a courtesy but are GitHub-only (neither
  is a CRAN reverse dependency at present).
