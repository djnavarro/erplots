...
`build_model_spaghetti()`; the default builder is used here:
...
`er_plot_show_data()` adds the raw observations at their true `(exposure,
response)` coordinates via `build_data_overlay()`, the default and only
...
`build_data_boxjitter()` (the older, panel-based responders/non-responders
design covered in the [binary responses](plot-binary.html#build_data_overlay-vs--build_data_boxjitter)
...
multiple grouping variables and `build_group_violin()`; the default
