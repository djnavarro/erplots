...
| Summary | `.layer_summary()` | `config$p_value`, `config$corner_distance` | `p_value` is `NULL` unless a `model` was supplied to `er_plot_add_summary()` (from `er_summary()`); `corner_distance` is a named vector of four minimum distances (`top_left`/`top_right`/`bottom_left`/`bottom_right`), computed from `data`'s raw `(exposure, response)` points rescaled onto `[0, 1]`, used to place a label away from the observed data |
