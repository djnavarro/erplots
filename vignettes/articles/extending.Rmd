...
`er_tte_add_model()`/`er_tte_add_summary()` -- namespaced separately
from `er_plot()`'s own `"model"`/`"summary"` tags, since a TTE builder
and an `er_plot()` builder share neither a signature nor `config`
contents, so a builder written for one grammar should never silently
pass the tag check for the other. A builder tagged for the wrong one
errors immediately:
...
`?er_style_tte_summary`/`?er_style_tte_model` for each layer's own
