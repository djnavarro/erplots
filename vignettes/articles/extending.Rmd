...
as public API on `?er_builder`, alongside each layer's own
...
and `er_builder_quantile_pointrange()` alternatives (and their
...
a single wrapper function, `er_builder_tag()`, with one optional
...
Tagging it with `er_builder_tag()` fixes that. Here's a complete
...
rather than the stratification variable's label. `er_builder_tag(builder,
...
instead; `er_builder_tag(builder, y_role = "count")` tells `.polish_labels()`
...
built-in builder declares it -- `er_builder_quantile_errorbar()` is
tagged `layer = "quantile"`, `er_builder_group_violin()` is tagged
...
for one layer's `summary_builder`-style secondary argument if that
...
`er_builder_tag()` call rather than four separate wrapper functions.
...
This is close to what the built-in `er_builder_data_hex()` does (it
...
correctly. See `?er_builder` for the full public-API contract, and
