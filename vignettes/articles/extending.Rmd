...
### One function, three independent arguments

`layout`, `fill_role`, and `y_role` are all set via the same
`er_builder_tag()` call rather than three separate wrapper functions.
...
```r
my_density_builder <- er_builder_tag(my_density_builder, layout = "overlay", fill_role = "density")
```

This is exactly what the built-in `er_builder_data_hex()` does.
