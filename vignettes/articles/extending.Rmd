...
- `ci_lower` and `ci_upper` are the confidence interval bounds. Our bulder will need these too.
...
bar. If we we wanted to use labels in our plot we would need these columns, but since we plan to skip that we can ignore them.
...
I am not at all convinced this is a good way to create an exposure-response plot: in statistical graphics, the "cross bar" visual idiom is almost always used to show distributional information (i.e., it feels like a boxplot without the whiskers). Using it to display a mean and confidence interval is almost certainly going to confuse anyone looking at your plot. Even so, it's convenient to illustrate how to design a builder function.
