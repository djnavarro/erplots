...
one topic that article only introduces: the `style` escape hatch
itself.
...
drawing to a `style` argument (`er_plot_add_model()` additionally has
`summary_style`), which defaults to one built-in `er_style_*()`
...
as public API on `?er_style`, alongside each layer's own
`er_style_*()` family page (`?er_style_model`, `?er_style_quantile`,
etc.).

### Passing extra arguments to a builder

Every `er_plot_add_*()` function also takes its own `...`, forwarded
unchanged to `style` (and, for `er_plot_add_model()`, `summary_style`)
when it's called at build time. Extra arguments must be named -- they're
...
|---|---|---|---|
| Model | `.layer_model()` | `config$predictions` | One row per exposure grid point, with `fit_resp`, `ci_lower`, `ci_upper` (from `er_predict()`) |
...
Suppose the built-in `er_style_quantile_errorbar()` (point + error bar)
and `er_style_quantile_pointrange()` alternatives (and their
bin-boundary-annotated `_vlines` variants) all feel like the wrong idiom,
and you'd rather draw the
per-bin summary as a `geom_crossbar()`. First, look at what
`config$summary` actually contains, by building the quantile layer on its
...
label geom that `er_style_quantile_errorbar()` draws alongside its error
...
it's a different slot from `style` on that same layer function).
...
`er_plot_add_*()` it's passed to (this is also true of a builder tagged
for one layer's `summary_style`-style secondary argument if that
layer doesn't have one; there's currently only `"summary"`, specific to
