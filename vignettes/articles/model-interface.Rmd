...
- **`newdata` is *returned*, not just used.** `er_predict()`'s contract is
  ...
  `.get_model_predictions()` (the internal caller, in
  `R/er-plot-layer.R`) builds `newdata` as an exposure grid crossed with
  every stratum level when the plot is stratified, and expects that
  column to survive the round trip through `er_predict()` unchanged.
...
(`er_style_model_spaghetti()`, `er_vpc_plot()`) checks for `NULL` and
treats it as "simulation isn't available for this model", not an error.
...
`er_vpc_plot(model = ...)` will refuse to run (with an informative error,
not a silently-too-narrow VPC band) if `sim_resp` is missing.
...
spaghetti plot above would still work unchanged, but the `er_vpc_plot()`
