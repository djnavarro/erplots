...
The default builder is `er_builder_model_ribbonline()`, but you can also draw
spaghetti plots to represent parameter uncertainty with
`er_builder_model_spaghetti()`. Spaghetti plots require the model to
implement `er_simulate()` (erglm's models do); models that only implement
`er_predict()` fall back to `er_builder_model_ribbonline()` with a message.
...
`er_builder_data_boxjitter()` is the older, panel-based design, and is
...
continuous/count response; `er_builder_data_overlay()` (raw points at their
...
color-encoded panel) remains possible via [er_builder_layout()] if a project
...
belongs to via [er_builder_layout()], which is what `er_plot_add_data()` uses
...
for `er_builder_data_boxjitter()`, sharing the model curve's own legend, the
same way `er_builder_data_overlay()`'s color aesthetic does for any response
type:
...
The default builder is `er_builder_group_boxplot()`, but you can also use
violin plots with `er_builder_group_violin()`:
