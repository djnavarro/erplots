...
instead of a rate with a Clopper-Pearson interval. The data strip
...
`er_plot_show_datastrip()` raises an error for a continuous-response
`er_plot` rather than silently mis-plotting:

```{r strip-1, error = TRUE}
erglm_data |> 
  er_plot(aucss, biomarker_change) |> 
  er_plot_show_model(mod_gaussian) |> 
  er_plot_show_datastrip()
