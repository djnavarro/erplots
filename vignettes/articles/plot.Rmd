...
`build_data_jitter()` (binary response) and `build_data_color()`
...
`er_plot_show_data()` uses to decide whether to merge it into the main
panel or stack it in panels below.
...
and again for a continuous response, where `build_data_color()` falls
back to a single response-colored panel below the model curve rather
