...
[count](plot-count.html) response articles; for writing a custom
builder in detail, see [Extending erplots](extending.html). If a design
choice described here ever changes, this article and `PLAN.md`'s
...
argument (`er_plot_add_model()` additionally has `summary_builder`)
sharing a common signature -- `function(data, config, stratify,
exposure, response, strata, style)`. That signature is a documented,
public part of the API (see [er_partial()]), each layer's `builder`
defaults to one built-in `er_builder_*()` function, and it can be set to
any other function matching the same signature -- no need to fork the
package or reach into `object$part` internals. For the data layer
...
builder-metadata helpers ([er_builder_layout()], [er_builder_fill_role()],
[er_builder_y_role()]) a builder can use to tag itself for the
