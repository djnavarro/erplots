...
#' `plot_by`'s type), a dodged point/interval per categorical bin,
#' continuous-x percentile bands, or a dodged point/interval per bin
#' *and* per requested percentile.
#'
...
#'   error bars, of `er_style_vpc_simulated_mean_errorbar()`'s when
...
er_style_vpc_simulated_quantile_errorbar <- function(data, config, exposure, response, theme,
  ...
  if (is.null(config$percentiles)) {
    rlang::abort(c(
      ...
      "i" = "Use `er_style_vpc_simulated_errorbar()` or `er_style_vpc_simulated_mean_errorbar()` instead for a binary response."
    ))
  }
  list(
    ggplot2::geom_errorbar(
